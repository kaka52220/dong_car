 ...
 * @brief   6x8 pixel ASCII font for SSD1306 OLED
 *
 *          Each character occupies 6 bytes (6 columns x 8 rows).
 *          Characters cover ASCII 0x20 (space) through 0x7E (~).
 *          Total: 95 characters = 570 bytes.
 */
...
#define FONT_END     0x7E   /* Last char: ~ */
#define FONT_COUNT   (FONT_END - FONT_START + 1)

/**
 * @brief   6x8 ASCII font lookup table
 * @note    Each byte column is read top-to-bottom (bit0 = top pixel).
 *          Columns per char: 6
 *          Index: (ch - 0x20) * 6
 */
static const uint8_t OLED_Font6x8[FONT_COUNT][FONT_WIDTH] = {
